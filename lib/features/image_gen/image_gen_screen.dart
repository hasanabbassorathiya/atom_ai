  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      if (!imageService.isImageInitialized || imageService.currentModelId != activeModels.imageModel!.id) {
        ...
        await imageService.initImage(
          modelId: activeModels.imageModel!.id,
          modelPath: modelPath,
        );
      }
      ...
      if (mounted) {
        setState(() => _imageData = bytes.toList());
      }
    } catch (e) {
      if (mounted) {
                        ...
                        FilledButton.icon(
                          onPressed: () {
                             SharePlus.instance.share(ShareParams(files: [
                               XFile.fromData(Uint8List.fromList(_imageData!), mimeType: 'image/png', name: 'generation.png')
                             ], text: _currentPrompt));
                          },
                        ...
                        child: Column(
                          ...
                          children: [
                            Icon(Icons.image_search_outlined,
                                size: 80, color: theme.colorScheme.outline.withValues(alpha: 0.5)),
                            const SizedBox(height: 24),
                            Text(
                              'Create anything',
